...
// Социальные иконки, заменено на LinkedIn
import IconLinkedin from "~icons/mdi/linkedin";
import IconGithub from "~icons/mdi/github";
import IconTelegram from "~icons/mdi/telegram";

export const Contact = () => {
  // Состояния для управления модальным окном и статусом отправки
  const [isOpen, setIsOpen] = useState(false);
  const [email, setEmail] = useState("");
  const [message, setMessage] = useState("");
  const [status, setStatus] = useState("");

  // Явно указали тип события e: React.FormEvent<HTMLFormElement>
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEmail("");
        setMessage("");
        setTimeout(() => setIsOpen(false), 2000); // Закрываем окно через 2 секунды
      } else {
        setStatus(data.message || "Ошибка при отправке");
      }
    } catch (error) {
    ...
    <section id="contact" className="py-32">
      <div className="max-w-2xl mx-auto px-6 group-3d perspective-1200">
        <div className="card-3d p-8 md:p-12 rounded-3xl border border-neutral-800/80 bg-[#050505]/60 backdrop-blur-xl relative overflow-hidden shadow-2xl hover:border-purple-500/30 hover:shadow-[0_40px_80px_-20px_rgba(168,85,247,0.2)]">
          {/* Обновленный класс градиента с bg-linear-to-br вместо bg-gradient-to-br */}
